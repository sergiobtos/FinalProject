...
export default function Alerts() {
  ...
  const [alerts, setAlerts] = React.useState(
    [{ _id: "", patientId: "", patientUserName: "", status: true, time: Date.now(), message : "" }]
  );
  ...
  var handleAlert = async (alertId: string) => {
    ...
    const jsonResult = await res.json(); 
        if (jsonResult.msg === 1) {
          setButtonClicked({
            isClicked : true,
            message : `Check ${jsonResult.patientUsername[0].patientUserName.toUpperCase()} Now !`
          })
          retrieveAlerts();
        } else {
          setButtonClicked({
      ...
      <Table className={classes.table} aria-label="simpple table">
        <TableHead>
          {buttonClicked.isClicked && (
              <TableRow>
                <TableCell className={classes.tableCell}>{ buttonClicked.message}</TableCell>
              </TableRow>)
          }
          <TableRow>
        ...
        <TableBody>
          {
            alerts.map((alert)=> (
                <TableRow key={alert._id}>
                  ...
                  <TableCell  > {alert.message}</TableCell>
                  <TableCell  > <Button > <IconButton onClick={ () => handleAlert(alert._id)}>
